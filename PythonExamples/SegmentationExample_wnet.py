# Trying a "w-net" (a la multigrid) instead of "U-net".
...
# removing some of the layers. Maybe increase again when we have more
# data and when loss performance is better.
def build_wnet_model(min_height, min_width):
    print('building wnet model')
    inputs = layers.Input(shape=(min_height,min_width,3))
    f1, p1 = downsample_block(inputs, 64)
    ...
    f7, p7 = downsample_block(u6, 512)
    print('f7 shape ', f7.shape)
    print('p7 shape ', p7.shape)
    bottleneck = double_conv_block(p7, 1024)
    print('bottleneck shape ', bottleneck.shape)
    u7 = upsample_block(bottleneck, f7, 512)
    ...
    wnet_model = keras.Model(inputs, outputs, name="W-Net")
    return wnet_model


# Build the u-net model.
if run_unet:
    model = build_unet_model(min_height, min_width)

if run_wnet:
    model = build_wnet_model(min_height, min_width)
