...
def getmodel(**kwargs):
    ...
    if model_type == "unet_2d":
        return models.unet_2d(
            (im_width, im_height, 3), n_labels=1, pool="max", unpool='nearest', filter_num=filter_num, **kwargs, name="unet"
        )

    if model_type == "unet++":
        return models.unet_plus_2d(
            (im_width, im_height, 3), filter_num, n_labels=1, **kwargs, pool='max', unpool=False, deep_supervision=False, name='xnet'
            )